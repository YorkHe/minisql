...
statement Read_Interpreter(char input[])
{
	...
	else if (!strcmp(Input_splitp[0].c_str(), "insert"))
	{
		if (!strcmp(Input_splitp[1].c_str(), "into") && !strcmp(Input_splitp[3].c_str(), "values"))
		{
			...
			for (int j = 4; j < i; j++)
			{
				...
				cout << Input_splitp[j] << endl;

				SQL.statement3.Insert_values[SQL.statement3.Values_num].value = Input_splitp[j].c_str();
				...
				else
				{
					...
					SQL.statement3.Insert_values[SQL.statement3.Values_num].value1 = atoi(Input_splitp[j].c_str());
					SQL.statement3.Values_num++;
				}
			}
			return SQL;
	...
	else if (!strcmp(Input_splitp[0].c_str(), "create"))
	{
		...
		{		
			cout << Input_splitp[3] << "|||" << Input_splitp[i - 1] << endl;
