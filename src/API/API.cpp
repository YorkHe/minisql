...
void Excute_SQL(statement SQL, string& name_of_database) //结合Record Manager，Index Manager，Catalog Manage模块提供的接口执行语句
{
	...
	else if (SQL.num == 2)//删除记录
	{
		...
		else//根据where子句删除特定记录
		{
			...
			{
				Formula tmp_where;
				tmp_where.left = SQL.statement2.delete_condition[i].column;
				tmp_where.right = SQL.statement2.delete_condition[i].value;
				...
				{
					tmp_where.relation = LESS_AND_EQUAL;
				}
				else if (SQL.statement2.delete_condition[i].op == ">=")
				{
					tmp_where.relation = GREATER_AND_EQUAL;
				}
				else if (SQL.statement2.delete_condition[i].op == "<>")
				{
					tmp_where.relation = NOT_EQUAL;
				}
	...
	else if (SQL.num == 1)//选择语句
	{
		...
		if (SQL.statement1.condition_num == 0)
		{
			...
			try
			{
				Is_Error = record.selectRecord(star,cond);
				if (Is_Error.err_code)
				{
					cout << Is_Error.description;
				}
		...
		else
		{
			...
			{
				Formula temp_where;
				temp_where.left = SQL.statement1.select_condition[i].column;
				temp_where.right = SQL.statement1.select_condition[i].value;
				if (SQL.statement1.select_condition[i].op == "=")
				{
					temp_where.relation= EQUAL;
				}
				else if (SQL.statement1.select_condition[i].op == ">")
				{
					temp_where.relation= GREATER;
				}
				else if (SQL.statement1.select_condition[i].op == "<")
				{
					temp_where.relation= LESS;
				}
				else if (SQL.statement1.select_condition[i].op == "<=")
				{
					temp_where.relation= LESS_AND_EQUAL;
				}
				else if (SQL.statement1.select_condition[i].op == ">=")
				{
					temp_where.relation= GREATER_AND_EQUAL;
				}
				else if (SQL.statement1.select_condition[i].op == "<>")
				{
					temp_where.relation= NOT_EQUAL;
				}
